...
onmessage = function domcryptWorkerOnMessage(aEvent)
{
  ...
  case INITIALIZE:
    ...
    break;
  case SHA256HASH:
    result = WeaveCryptoWrapper.sha256Hash(aEvent.data.stringToHash);
  ...
  case DECRYPT:
    let cipherMessage = {
      ...
      _windowID: aEvent.data._windowID,
    };
  ...
  case HIDE:
    ...
    let signature = WeaveCryptoWrapper.sign(hash,
                                            aEvent.data.passphrase,
                                            aEvent.data.privKey,
                                            aEvent.data.iv,
                                            aEvent.data.salt);
  ...
  case SHOW:
    log("SHOW");
    pprint(aEvent.data.metaData);
    ///////////////
    // create hash:

    let _hash = WeaveCryptoWrapper.sha256Hash(aEvent.data.cipherContent);
    log("_hash created........ " + _hash);

    ///////////////////
    // verify signature

    let verification = WeaveCryptoWrapper.verify(_hash,
                                                 aEvent.data.signature,
    ...
    let decryptCipherMessage = {
      content: aEvent.data.cipherContent,
      wrappedKey: aEvent.data.cipherWrappedKey,
      pubKey: aEvent.data.cipherPubKey,
      iv: aEvent.data.cipherIV,
      signature: aEvent.data.signature,
      verification: verification,
    };

    result = WeaveCryptoWrapper.decrypt(decryptCipherMessage,
                                        aEvent.data.passphrase,
                                        aEvent.data.privKey,
                                        aEvent.data.salt,
                                        aEvent.data.iv);
...
var WeaveCryptoWrapper = {
  ...
  sha256Hash: function WCW_sha256Hash(aStringToHash)
  {
    let SHA256 = 4;
    try {
      let result = WeaveCrypto.hash(SHA256, aStringToHash);
  ...
  encrypt: function WCW_encrypt(aPlainText, aPublicKey)
  {
    ...
    try {
      ...
      return { content: cryptoMessage,
               pubKey: aPublicKey,
               wrappedKey: wrappedKey,
               iv: aIV,
               type: "encryptedData",
               format: "DER_BASE64", // XXX: need to offer a JWK option?
             };
    }
    catch (ex) {
      log(ex);
  ...
  function WCW_decrypt(aCipherMessage, aPassphrase, aPrivateKey, aSalt, aIV)
  {
    try {
      ...
      decryptedMsg._windowID = aCipherMessage._windowID;
...
const kAllowedHashAlgs = { 4: "SHA256",
                           5: "SHA384",
                           6: "SHA512",
                           7: "SHA224",
                         };

const kHashAlgsResultLength = {
  "SHA256": 32,
  "SHA224": 28,
  "SHA384": 48,
  "SHA512": 64,
...
var WeaveCrypto = {
  ...
  initNSS : function WC_initNSS(aNSSPath) {
    ...
    // security/nss/lib/util/hasht.h#13
    // typedef enum {
    //  HASH_AlgNULL   = 0,
    //  HASH_AlgMD2    = 1,
    //  HASH_AlgMD5    = 2,
    ...
    this.nss_t.HASH_HashType = ctypes.int; // use 4 for SHA256


    // SECStatus HASH_HashBuf(HASH_HashType type, unsigned char *dest,
    //                         const unsigned char *src, PRUint32 src_len)
    // security/nss/lib/cryptohi/sechash.c#297

    this.nss.HASH_HashBuf = nsslib.declare("HASH_HashBuf",
                                           ctypes.default_abi,
                                           this.nss_t.SECStatus,
                                           this.nss_t.HASH_HashType,
                                           ctypes.unsigned_char.ptr,
                                           ctypes.unsigned_char.ptr,
                                           ctypes.int);
    // Example: HASH_Buf(4, unsignedSrcCharPtrArr,
    //                   unsignedDestCharPtrArr, unsignedSrcCharPtr.length);
  ...
  hash : function _hash(hashType, srcData)
  {
    log("WC hash()");
    if (!(hashType in kAllowedHashAlgs)) {
      throw new Error("hash() failed! Unknown HashType: " + hashType);
    }
    let hLength = kAllowedHashAlgs[hashType];
    let resultLength = kHashAlgsResultLength[hLength];
    log("hLength: " + hLength);
    log("resultLength: " + resultLength);
    if (!resultLength) {
      throw new Error("hash() failed, unknown kHashAlgsResultLength");
    }
    // XXX: catch raised errors, send error message to worker

    let srcArr = new ctypes.ArrayType(ctypes.unsigned_char, srcData.length)();
    ...
    log("destArr.length: " + destArr.length);
    return this.encodeBase64(destArr.address(), destArr.length);
  },
